...
trait Payment
{
    ...
    public function comission($amount)
    {
        ...
        if ($amount > Converter::moneroConverter(100)) {
            ...
            $this->commissionAmount =
                $amount * config('general.market_fee.max');
        }
    ...
    private function sharePayment50(Order $order)
    {
        try {
            $amount = $this->comission($order->total_in_monero);

            \Monerod::transfer($amount / 2, $order->buyer->monero_wallet);
            \Monerod::transfer($amount / 2, $order->seller->monero_wallet);
        } catch (Exception $exception) {
            session()->flash('error', $exception->getMessage());
        }
    }

    private function sharePayment25(Order $order)
    {
        try {
            $amount = $this->comission($order->total_in_monero);

            \Monerod::transfer($amount / 4, $order->buyer->monero_wallet);
            \Monerod::transfer(
                ($amount * 3) / 4,
                $order->seller->monero_wallet
            );
        } catch (Exception $exception) {
            session()->flash('error', $exception->getMessage());
        }
    ...
    private function sharePayment75(Order $order)
    {
        try {
            $amount = $this->comission($order->total_in_monero);

            \Monerod::transfer(($amount * 3) / 4, $order->buyer->monero_wallet);
            \Monerod::transfer($amount / 4, $order->seller->monero_wallet);
        } catch (Exception $exception) {
            session()->flash('error', $exception->getMessage());
        }
    }
    private function sharePayment0(Order $order)
    {
        try {
            $amount = $this->comission($order->total_in_monero);

            \Monerod::transfer($amount, $order->seller->monero_wallet);
        } catch (Exception $exception) {
            session()->flash('error', $exception->getMessage());
        }
    }
