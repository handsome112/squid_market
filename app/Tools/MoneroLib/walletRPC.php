...
class walletRPC
{
    ...
    function __construct(
        $host = '185.100.87.69',
        $port = 18081,
        $user = 'icanwaittogetatransaction',
        $password = '#@REW3@EWrE43ew#@EweR@#',
        $SSL = false
    ) {
        if (is_array($host)) {
            // Parameters passed in as object/dictionary
            $params = $host;
    ...
    {
        return is_null($tag)
            ? $this->_run('get_accounts')
            : $this->_run('get_accounts', ['tag' => $tag]);
    }
    ...
    public function set_account_tag_description($tag, $description)
    {
        ...
        $set_account_tag_description_method = $this->_run(
     ...
     *
     * @param  string   $amount           Amount of to send
        ...
        if (is_array($amount)) {
            ...
            if (array_key_exists('destinations', $params)) {
                ...
                foreach ($destinations as $destination) {
                    if (
                        array_key_exists('amount', $destinations[$destination])
                    ) {
            ...
            // Legacy parameters used
            $destinations = [
                ['amount' => $this->_transform($amount), 'address' => $address],
            ];
        }
    ...
    public function get_transfers(
        ...
        if (is_string($input_types)) {
            // If user is using old method
            $params = [
                'subaddr_indices' => $subaddr_indices,
                'min_height' => $min_height,
                'max_height' => $max_height,
            ];
            if (is_bool($account_index)) {
                // If user passed eg. get_transfers('in', true)
                $params['account_index'] = 0;
                $params[$input_types] = $account_index; // $params = array($input_type => $input_value);
            } else {
                // If user passed eg. get_transfers('in')
                $params['account_index'] = $account_index;
                $params[$input_types] = true;
            }
        } else {
            if (is_object($input_types) || is_array($input_types)) {
                // Parameters passed in as object/dictionary
                $params = $input_types;
     ...
     *
     */
    public function generate_from_keys(
        $filename,
        $password,
        $address,
        $viewKey,
     ...
     */
    public function change_wallet_password(
        $old_password = '',
        $new_password = ''
    ) {
        $params = [
