...
class daemonRPC
{
    ...
    public function is_key_image_spent($key_images)
    {
        ...
        if (!is_array($key_images)) {
            throw new Exception(
                'Error: key images must be an array or a string'
            );
        }
    ...
    public function send_raw_transaction(
        $tx_as_hex,
        $do_not_relay = false,
    ...
    public function start_mining(
        ...
        if ($threads_count < 0) {
            throw new Exception(
                'Error: threads_count must be a positive integer'
            );
        }
        $params = [
            'do_background_mining' => $background_mining,
