...
class User extends Authenticatable
{
    ...
    public function communityRates()
    {
        if ($this->totalFeedbacks() == 0) {
            return number_format(0, 1);
        }

        $totalrates = $this->feedbacks()->sum('community');
        $rates = $totalrates / $this->totalFeedbacks();
    ...
    public function shippingRates()
    {
        ...
        $totalrates = $this->feedbacks()->sum('shipping');
        $rates = $totalrates / $this->totalFeedbacks();
    ...
    public function productsRates()
    {
        ...
        $totalrates = $this->feedbacks()->sum('products');
        $rates = $totalrates / $this->totalFeedbacks();
    ...
    {
        return Carbon::parse($this->last_logoff)->diffForHumans();
    }
    ...
    public function balance_cash()
    {
        $balanceXMR = \Monerod::getBalance($this->id);
        $rates = Converter::currencyLatestPrice();
        $xmrRates = Converter::moneroLastPrice();

        $result = number_format(
            ($balanceXMR * $xmrRates * $rates[auth()->user()->currency]) /
                $rates['USD'],
            2
        );
        return $result;
